...
export class TextScene {
     ...
     @SceneEnter()
     async getAllVoices(ctx: IContext) {
          ...
          await this.extra.replyOrEditWithAudio(
               {
                    ...
                    filename: api.name,
                    text: `${info}\nEnter the <b>text</b> that will be <b>voiced</b> by that voice.`
               },
               ctx
          );
     }
     ...
     async onText(@Ctx() ctx: IContext, @Message() message: IMessage) {
          ...
          const api = await this.api.getTextToSpeech(message.text, ctx.session.voiceId);
